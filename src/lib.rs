...
pub fn unique_result_scan_domains(
    domains: Vec<String>,
    subs_flag: bool,
...
///
/// ```
pub fn scan_domains(domains: Vec<String>, subs_flag: bool, api_key: Option<String>) -> Vec<String> {
    let mut result: Vec<String> = vec![];

    let _ph = domains
        .iter()
        .map(|domain| result.append(&mut scan_domain(domain, subs_flag, &api_key)))
        .collect::<Vec<()>>();
...
pub fn unique_result_scan_domain(
    domain: &str,
    subs_flag: bool,
    api_key: &Option<String>,
) -> Vec<String> {
...
///
/// ```
pub fn scan_domain(domain: &str, subs_flag: bool, api_key: &Option<String>) -> Vec<String> {
    let mut result: Vec<String> = vec![];
...
pub fn get_wayback_url(domain: &str, subs_flag: bool) -> Option<Vec<String>> {
    ...
    match client()
        ...
        .send()
    {
        Ok(response) => match response.json::<Vec<Vec<String>>>() {
            Ok(response) => {
                let wayback_urls: Vec<String> = response
                    .into_iter()
                    .filter(|line| {
                        line != &vec![
                            "urlkey",
            ...
            match response.text() {
                Ok(response) => {
                    ...
                    let _ph = response
                        ...
                        .collect::<()>();
                    if !inner_urls.is_empty() {
                        urls = Some(inner_urls)
                    }
                }
                Err(err) => {
                    eprintln!("{:?}", err);
                            ...
                            match res_val.get("detected_urls") {
                                Some(detected_urls) => {
                                    match serde_json::from_value::<Vec<VT>>(detected_urls.clone()) {
                                        Ok(vts) => {
                                            urls = Some(vts.into_iter().map(|x| x.url).collect());
                                        }
                                        Err(err) => {
                                            eprintln!("{:?}", err);
                                        }
